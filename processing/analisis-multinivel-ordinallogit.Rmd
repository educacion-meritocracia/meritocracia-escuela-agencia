---
title: "analisis-multinivel-ordinallogit"
...
reg1_pension <- clmm(just_pension_fact ~ 1 + esfuerzo_soc + (1 | mrbd), data=data_pension)
reg2_pension <- clmm(just_pension_fact ~ 1 + merito_soc + (1 | mrbd), data=data_pension)
reg3_pension <- clmm(just_pension_fact ~ 1 + inteligencia_soc + (1 | mrbd), data=data_pension)
reg4_pension <- clmm(just_pension_fact ~ 1 + esfuerzo_esc + (1 | mrbd), data=data_pension)
reg5_pension <- clmm(just_pension_fact ~ 1 + inteligencia_esc + (1 | mrbd), data=data_pension)
reg6_pension <- clmm(just_pension_fact ~ 1 + educacion_rec + (1 | mrbd), data=data_pension)
reg7_pension <- clmm(just_pension_fact ~ 1 + libros_rec + (1 | mrbd), data=data_pension)
reg8_pension <- clmm(just_pension_fact ~ 1 + acc_tec + (1 | mrbd), data=data_pension)
reg9_pension <- clmm(just_pension_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 | mrbd), data=data_pension)
```

```{r results='asis'}
htmlreg(list(reg0_pension, reg1_pension, reg2_pension, reg3_pension, reg4_pension, reg5_pension, reg6_pension, reg7_pension, reg8_pension, reg9_pension), 
...
reg1_salud <- clmm(just_salud_fact ~ 1 + esfuerzo_soc + (1 | mrbd), data=data_salud)
reg2_salud <- clmm(just_salud_fact ~ 1 + merito_soc + (1 | mrbd), data=data_salud)
reg3_salud <- clmm(just_salud_fact ~ 1 + inteligencia_soc + (1 | mrbd), data=data_salud)
reg4_salud <- clmm(just_salud_fact ~ 1 + esfuerzo_esc + (1 | mrbd), data=data_salud)
reg5_salud <- clmm(just_salud_fact ~ 1 + inteligencia_esc + (1 | mrbd), data=data_salud)
reg6_salud <- clmm(just_salud_fact ~ 1 + educacion_rec + (1 | mrbd), data=data_salud)
reg7_salud <- clmm(just_salud_fact ~ 1 + libros_rec + (1 | mrbd), data=data_salud)
reg8_salud <- clmm(just_salud_fact ~ 1 + acc_tec + (1 | mrbd), data=data_salud)
reg9_salud <- clmm(just_salud_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 | mrbd), data=data_salud)
```

```{r results='asis'}
htmlreg(list(reg0_salud, reg1_salud, reg2_salud, reg3_salud, reg4_salud, reg5_salud, reg6_salud, reg7_salud, reg8_salud, reg9_salud), 
        custom.model.names = c("Nulo",
                               "Modelo 1",
                               "Modelo 2",
                               "Modelo 3",
...
reg1_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + (1 | mrbd), data=data_redis)
reg2_redis <- clmm(redistribucion_fact ~ 1 + merito_soc + (1 | mrbd), data=data_redis)
reg3_redis <- clmm(redistribucion_fact ~ 1 + inteligencia_soc + (1 | mrbd), data=data_redis)
reg4_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_esc + (1 | mrbd), data=data_redis)
reg5_redis <- clmm(redistribucion_fact ~ 1 + inteligencia_esc + (1 | mrbd), data=data_redis)
reg6_redis <- clmm(redistribucion_fact ~ 1 + educacion_rec + (1 | mrbd), data=data_redis)
reg7_redis <- clmm(redistribucion_fact ~ 1 + libros_rec + (1 | mrbd), data=data_redis)
reg8_redis <- clmm(redistribucion_fact ~ 1 + acc_tec + (1 | mrbd), data=data_redis)
reg9_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 | mrbd), data=data_redis)
```

```{r results='asis'}
htmlreg(list(reg0_redis, reg1_redis, reg2_redis, reg3_redis, reg4_redis, reg5_redis, reg6_redis, reg7_redis, reg8_redis, reg9_redis), 
        custom.model.names = c("Nulo",
                               "Modelo 1",
                               "Modelo 2",
                               "Modelo 3",
...
```{r}
reg10_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec*esfuerzo_soc + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + educacion_rec | mrbd), data=data_redis)
reg11_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec*merito_soc + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + educacion_rec | mrbd), data=data_redis)
reg12_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec*inteligencia_soc + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + educacion_rec | mrbd), data=data_redis)
reg13_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec*esfuerzo_esc + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + educacion_rec | mrbd), data=data_redis)
reg14_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec*inteligencia_esc + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + educacion_rec | mrbd), data=data_redis)
```
...
```{r}
reg15_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec*esfuerzo_soc + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + libros_rec | mrbd), data=data_redis)
reg16_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec*merito_soc + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + libros_rec | mrbd), data=data_redis)
reg17_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec*inteligencia_soc + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + libros_rec | mrbd), data=data_redis)
reg18_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec*esfuerzo_esc + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + libros_rec | mrbd), data=data_redis)
reg19_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec*inteligencia_esc + acc_tec + cod_depe2 + cod_grupo + simce + internet_rec + (1 + libros_rec | mrbd), data=data_redis)
```
...
```{r}
reg20_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + cod_grupo*esfuerzo_soc + simce + internet_rec + (1 + cod_grupo | mrbd), data=data_redis)

reg21_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + cod_grupo*merito_soc + simce + internet_rec + (1 + cod_grupo | mrbd), data=data_redis)

reg22_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + cod_grupo*inteligencia_soc + simce + internet_rec + (1 + cod_grupo | mrbd), data=data_redis)

reg23_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + cod_grupo*esfuerzo_esc + simce + internet_rec + (1 + cod_grupo | mrbd), data=data_redis)

reg24_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + cod_grupo*inteligencia_esc + simce + internet_rec + (1 + cod_grupo | mrbd), data=data_redis)
```

```{r results='asis'}
screenreg(list(reg20_redis, reg21_redis, reg22_redis, reg23_redis, reg24_redis))
```

*meritocracia x simce*

```{r}
reg25_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce*esfuerzo_soc + internet_rec + (1 + simce | mrbd), data=data_redis)
reg26_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce*merito_soc + internet_rec + (1 + simce | mrbd), data=data_redis)
reg27_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce*inteligencia_soc + internet_rec + (1 + simce | mrbd), data=data_redis)
reg28_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce*esfuerzo_esc + internet_rec + (1 + simce | mrbd), data=data_redis)
reg29_redis <- clmm(redistribucion_fact ~ 1 + esfuerzo_soc + merito_soc + inteligencia_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + cod_depe2 + cod_grupo + simce*inteligencia_esc + internet_rec + (1 + simce | mrbd), data=data_redis)
```