...
# independientes
```{r}
independientes <- data %>% dplyr::select(inteligencia_esc, esfuerzo_esc, esfuerzo_soc, merito_soc, inteligencia_soc, educacion_rec, libros_rec, acc_tec)
...
df2<- dfSummary(independientes,
               ...
               graph.col = T, # plot
               valid.col = T, # n valido
               col.widths = c(20,1000,10,10,10,10))

df2$Variable <- c("School talent", "School effort", "Social effort", "Deservingness", "Social talent", "Education", "Books", "Technology")
...
df3<- dfSummary(school,
               ...
               graph.col = T, # plot
               valid.col = T, # n valido
               col.widths = c(10,1000,10,10,10,10))
...
## dependientes
```{r}
dependientes <- sjPlot::plot_stackfrq(select(data, just_educ, just_pension, just_salud, redistribucion), geom.size = 0.4) +
  theme_bw() +
...
```{r}
independientes <- sjPlot::plot_stackfrq(select(data, inteligencia_esc, esfuerzo_esc, inteligencia_soc, merito_soc, esfuerzo_soc)) +
  theme_bw() +
  theme(legend.position="bottom")
