...
```{r}
pacman::p_load(haven, dplyr, summarytools, sjmisc, car, sjlabelled, webshot)
...
# Dependientes
```{r}
dependientes <- data %>% dplyr::select(just_pension, just_educ, just_salud, redistribucion)
...
dependientes$just_pension <- set_label(dependientes$just_pension, label = "It is fair that in Chile people with higher incomes can have better pensions than people with low incomes")
...
dependientes$just_salud <- set_label(dependientes$just_salud,label = "It is fair that in Chile people with higher incomes can access better health services than people with low incomes")
...
dependientes$just_educ <- set_label(dependientes$just_educ,label = "It is fair that in Chile people who can pay have a better education for their children")

dependientes$bienestar <- set_label(dependientes$bienestar,label = "index of access to social welfare")
...
dependientes$redistribucion <- set_label(dependientes$redistribucion,label = "The Chilean government should do something to reduce the income gap between rich and poor")

# Pendiente modificar categoríass de respuestas y las demás variables indep (traducir a inglés)
...
webshot(url ="output/tables/desc01.html" ,file ="output/tables/desc01.png")
```

# independientes
```{r}
independientes <- data %>% select(inteligencia_esc, esfuerzo_esc, esfuerzo_soc, merito_soc, inteligencia_soc, educacion_rec, libros_rec, acc_tec)
...
view(df2,file = "output/tables/desc02.html")
...
```{r}
school <- data %>% select(mean_educ, simce, cod_depe2, cod_grupo)
...
df3<- dfSummary(school,
               ...
               valid.col = T, # n valido
               col.widths = c(1000,10,10,10,10))
df3$Variable <- NULL # delete variable column
view(df3,file = "output/tables/desc03.html")
