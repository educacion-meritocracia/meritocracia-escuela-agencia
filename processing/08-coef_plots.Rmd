...
soc_effort = data_rec %>% filter(!is.na(esfuerzo_soc)) %>% 
  ...
  summarise(education = mean(just_educ, na.rm = T),
            pension = mean(just_pension, na.rm = T),
            health = mean(just_salud, na.rm = T))

soc_effort <- soc_effort %>% pivot_longer(c("health", "education", "pension"), 
                              names_to="justice", 
                              values_to="mean")
...
deservingness <- deservingness %>% pivot_longer(c("health", "education", "pension"), 
                              names_to="justice", 
                              values_to="mean")
...
soc_talent <- soc_talent %>% pivot_longer(c("health", "education", "pension"), 
                              names_to="justice", 
                              values_to="mean")
...
school_effort <- school_effort %>% pivot_longer(c("health", "education", "pension"), 
                              names_to="justice", 
                              values_to="mean")
...
school_talent <- school_talent %>% pivot_longer(c("health", "education", "pension"), 
                              names_to="justice", 
                              values_to="mean")
```
...
diag(merit_cor) = NA
rownames(merit_cor) <- c("A. Pensions",
                         "B. Education",
                         "C. Health",
                         "D. Social effort",
                         "E. Social talent",
...
reg2_pension <- clmm(just_pension_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + (1 | mrbd), data=data_pension)
reg5_pension <- clmm(just_pension_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + internet_rec + mean_educ + cod_depe2 + cod_grupo + simce + (1 | mrbd), data=data_pension)
```
...
reg2_salud <- clmm(just_salud_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + (1 | mrbd), data=data_salud)
reg5_salud <- clmm(just_salud_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + internet_rec + mean_educ + cod_depe2 + cod_grupo + simce + (1 | mrbd), data=data_salud)
```
...
reg2_educ <- clmm(just_educ_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + (1 | mrbd), data=data_educ)
reg5_educ <- clmm(just_educ_fact ~ 1 + esfuerzo_soc + inteligencia_soc + merito_soc + esfuerzo_esc + inteligencia_esc + educacion_rec + libros_rec + acc_tec + internet_rec + mean_educ + cod_depe2 + cod_grupo + simce + (1 | mrbd), data=data_educ)
```
...
```{r}
plots2<-plot_models(reg5_bienestar, reg5_salud, reg5_educ, reg5_pension,  
            rm.terms = c("1|2", "2|3", "3|4", "educacion_recEd Media", "educacion_recEd Tecnica", "educacion_recUniversidad o posgrado", "educacion_recNs/Nr", "libros_recMás de 25", "acc_tec", "internet_rec2", "internet_recNs/Nr", "mean_educ", "cod_depe2Part. subvencionado", "cod_depe2Part. privado", "cod_grupoMedio bajo", "cod_grupoMedio", "cod_grupoMedio alto", "cod_grupoAlto"), #quitar terminos
            title = "", #quitar titulo
            show.values = TRUE, #mostrar valor de efectos
            ...
            show.legend = FALSE,
            m.labels = c("Index", "Health", "Education","Pensions")) + # variables dependientes
