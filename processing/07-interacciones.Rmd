...
```{r}
pacman::p_load(haven, dplyr, summarytools, sjmisc, car, sjlabelled, sjPlot, lme4, stargazer, corrplot, ltm, texreg, ordinal, MASS, webshot, gtools, kableExtra)
load("input/data/proc/data_rec.RData")

data_rec <- data_rec %>% rowwise() %>%  mutate(educacion_rec2 = case_when(educacion_rec=="Ed Basica"~"Baja educacion",
                                                 educacion_rec=="Ed Media"~"Baja educacion",
                                                 educacion_rec=="Ed Tecnica"~"Baja educacion",
                                                 educacion_rec=="Universidad o posgrado"~"Universidad o posgrado",
                                                 educacion_rec=="Ns/Nr"~"Ns/Nr"
                                                 ))
data_rec$educacion_rec2 <- factor(data_rec$educacion_rec2, levels = c("Baja educacion", "Universidad o posgrado", "Ns/Nr"))
...
interact1_1 <- cbind(interact1@coef[19:20], interact1@pvalues[19:20])

interact2 <- texreg::extract(reg11_bienestar)
interact2_1 <- cbind(interact2@coef[19:20], interact2@pvalues[19:20])

interact3 <- texreg::extract(reg12_bienestar)
interact3_1 <- cbind(interact3@coef[19:20], interact3@pvalues[19:20])

interact4 <- texreg::extract(reg13_bienestar)
interact4_1 <- cbind(interact4@coef[19:20], interact4@pvalues[19:20])

interact5 <- texreg::extract(reg14_bienestar)
interact5_1 <- cbind(interact5@coef[19:20], interact5@pvalues[19:20])
...
kbl(collapse_cols, 
  ...
  add_header_above(c(" " = 1,
                     "Social welfare index"=5, 
                     "Redistribution"=5), 
                   bold = T)%>%  
  kable_styling(latex_options = c("scale_down","HOLD_position"),
                full_width = F) %>%
  column_spec(1, bold = T) %>%
  column_spec(column = 2:11,width = "1.5cm") %>%
  collapse_rows(columns = 1:2, valign = "middle") %>% 
  footnote(general = "***p < 0.001, **p < 0.01, *p < 0.05",
           footnote_as_chunk = T,general_title = "Note: ") %>% 
  save_kable("output/tables/interactions2.png")
```
